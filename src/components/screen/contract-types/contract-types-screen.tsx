...
export default function ContractTypesScreen() {
  ...
  const fetchContractTypes = useCallback(async (background = false) => {
    ...
    try {
      ...
      const res = await fetch(`/api/contract-types?${params}`)
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('Tipe kontrak berhasil dibuat', 'success')
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true)
    try {
      const res = await fetch(`/api/contract-types/${selectedItem.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true)
    try {
      const res = await fetch(`/api/contract-types/${selectedItem.id}`, { method: 'DELETE' })
      const json = await res.json()
    ...
    <div className="flex flex-col gap-6 pb-20 p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-gray-200 pb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-[#1C2A55]">Tipe Kontrak</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola jenis-jenis kontrak kerjasama.</p>
        </div>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-4 py-16 text-center text-gray-400">
                    ...
                    <p className="text-xs mt-1">Silakan tambah tipe kontrak baru.</p>
                  </td>
                </tr>
              ) : (
                data.map((item) => (
          ...
          <div className="w-full max-w-sm rounded-lg bg-white shadow-xl animate-in zoom-in-95 duration-200 ring-1 ring-gray-100">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
              <h3 className="text-lg font-bold text-[#1C2A55]">
                {isCreateOpen ? 'Tambah Tipe Kontrak' : 'Edit Tipe Kontrak'}
              </h3>
              <button 
            ...
            <form onSubmit={isCreateOpen ? handleCreate : handleUpdate} className="p-5 space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Nama Tipe Kontrak</label>
                <input 
                  required
                  autoFocus
                  type="text" 
                  ...
                  placeholder="Contoh: Perjanjian Kerjasama, MoU"
                />
              </div>
