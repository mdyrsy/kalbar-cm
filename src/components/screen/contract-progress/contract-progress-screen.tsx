...
import { 
  Search, Plus, TrendingUp, ArrowUpDown, 
...
export default function ContractProgressScreen() {
  const { showToast } = useToast()
  
  // State Data
  const [data, setData] = useState<ContractProgress[]>([])
  ...
  // Form Selection
  const [selectedItem, setSelectedItem] = useState<ContractProgress | null>(null)
  const [formData, setFormData] = useState({ name: '', description: '' })

  // 1. Fetch Data
  const fetchProgress = useCallback(async (background = false) => {
    ...
    try {
      ...
      // PERBAIKAN: Menggunakan endpoint plural 'contract-progresses'
      const res = await fetch(`/api/contract-progresses?${params}`)
      
      // Cek Content-Type untuk mencegah error HTML
      const contentType = res.headers.get("content-type");
      if (!contentType || !contentType.includes("application/json")) {
        throw new Error("API Path Salah atau Server Error (404/500)")
      }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // PERBAIKAN: Endpoint plural
      const res = await fetch('/api/contract-progresses', {
        method: 'POST',
      ...
      showToast('Status progress berhasil dibuat', 'success')
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      // PERBAIKAN: Endpoint plural
      const res = await fetch(`/api/contract-progresses/${selectedItem.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          name: formData.name,
  ...
  const handleDelete = async () => {
    ...
    try {
      // PERBAIKAN: Endpoint plural
      const res = await fetch(`/api/contract-progresses/${selectedItem.id}`, { method: 'DELETE' })
      const json = await res.json()
  ...
  const openEdit = (item: ContractProgress) => {
    ...
    <div className="flex flex-col gap-6 pb-20 p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-gray-200 pb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-[#1C2A55]">Progress Kontrak</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola status dan tahapan (milestone) kontrak.</p>
        </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between border-b border-gray-100">
          <div className="relative w-full md:max-w-xs">
            ...
            <input 
              type="text"
              placeholder="Cari status progress..."
              value={search}
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 text-gray-500">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Deskripsi</th>
                <th 
                  className="px-4 py-3 font-semibold text-xs uppercase tracking-wider cursor-pointer hover:text-[#1C2A55] transition-colors"
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-16 text-center text-gray-400">
                    ...
                    <p className="text-xs mt-1">Silakan tambah status progress baru.</p>
                  </td>
                </tr>
              ) : (
                // Data Rows
          ...
          <div className="w-full max-w-sm rounded-lg bg-white shadow-xl animate-in zoom-in-95 duration-200 ring-1 ring-gray-100">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
              <h3 className="text-lg font-bold text-[#1C2A55]">
                {isCreateOpen ? 'Tambah Status' : 'Edit Status'}
              </h3>
              <button 
            ...
            <form onSubmit={isCreateOpen ? handleCreate : handleUpdate} className="p-5 space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Nama Progress</label>
                <input 
                  required
                  autoFocus
                  type="text" 
                  ...
                  placeholder="Contoh: Menunggu Review"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Deskripsi (Opsional)</label>
                <textarea 
                  rows={3}
                  value={formData.description}
                  onChange={e => setFormData({ ...formData, description: e.target.value })}
                  className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm text-[#1C2A55] focus:border-[#1C2A55] focus:outline-none focus:ring-1 focus:ring-[#1C2A55] transition-all resize-none"
